...
exports.resetOllamaCheck = resetOllamaCheck;
exports.persistEnrichedGraph = persistEnrichedGraph;
exports.enrichWithSemanticContext = enrichWithSemanticContext;
 ...
 */
const USP_SYSTEM_PROMPT = "Distill this Swift code into a machine-to-machine logic flow.\n" +
 ...
 */
function buildLibraryPrompt(entry, callerNames) {
    const lines = [];
    lines.push(`Library: ${entry.name}`);
    if (entry.origin)
        lines.push(`Origin: ${entry.origin}`);
    if (entry.calls?.length)
        lines.push(`Depends on: ${entry.calls.slice(0, 5).join(", ")}`);
    if (callerNames.length > 0)
        lines.push(`Used by: ${callerNames.slice(0, 8).join(", ")}`);
    return lines.join("\n");
}
// ═══════════════════════════════════════════════════════════════════════════════
// CONTENT HASHING
 ...
 */
function extractSource(absPath, line) {
    if (!absPath || line <= 0)
        return null;
...
function countFullBody(lines, start) {
    let depth = 0;
    let foundOpen = false;
 ...
 */
function extractMergedObjectSource(entry) {
    // Primary definition
    ...
    if (blocks.length === 0)
        return { merged: null, extensionCount: 0 };
    return {
        merged: blocks.join("\n\n// --- extension ---\n\n"),
 ...
 */
function generateFallback(entry, sourceCode, childContexts, callerNames, extensionCount = 0) {
    const segments = [];
 ...
 */
function buildExecFlow(entry, sourceCode) {
 ...
 */
function inferCallOrder(callIds, sourceCode) {
    const indices = callIds.map((_, i) => i);
    if (!sourceCode || callIds.length <= 1)
        return indices;
    ...
    for (let idx = 0; idx < callIds.length; idx++) {
        ...
        if (!name) {
            positions.push({ idx, pos: Infinity });
            continue;
        }
 ...
 */
function buildAccessorFlow(execTag, propName, sourceCode, entry) {
    const flow = [];
    const callCount = entry.calls?.length ?? 0;
    ...
    if (execTag === "ws") {
        ...
        for (let ci = 0; ci < Math.min(callCount, 2); ci++)
            flow.push(`?c[${ci}]`);
        const rets = entry.returns?.slice(0, 2);
        if (rets?.length)
            flow.push(`$${rets.join(",")}`);
...
function inferAccessorStateChange(execTag, propName, triggers, sourceCode) {
    // Name-based state inference
    const prop = propName.toLowerCase();
    if (/blur|effect|opacity|alpha/.test(prop))
        return "blur_lvl";
    if (/color|theme|style/.test(prop))
        return "appearance";
    if (/frame|bounds|size|position|origin/.test(prop))
        return "layout";
    if (/text|title|label/.test(prop))
        return "display";
    if (/state|status|phase|mode/.test(prop))
        return "st_change";
    if (/count|total|sum/.test(prop))
        return "metric";
    if (/enabled|disabled|active|visible|hidden/.test(prop))
        return "ui_toggle";
    if (/selected|checked|on/.test(prop))
        return "selection";
    if (/url|path|endpoint/.test(prop))
        return "resource";
    if (/token|auth|session/.test(prop))
        return "auth";
    // Source-based: if triggers contain UI-related calls
    if (triggers.some((t) => /layout|display|render|refresh|update.*ui|redraw/i.test(t)))
        return "ui_refresh";
    if (triggers.some((t) => /save|persist|write|store/i.test(t)))
        return "persist";
    if (triggers.some((t) => /notify|post|send|delegate/i.test(t)))
        return "notify";
    if (triggers.some((t) => /connect|disconnect|reconnect/i.test(t)))
        return "conn";
 ...
 */
function buildObjectFlow(entry, sourceCode, childContexts, extensionCount) {
    const flow = [];
    ...
    const lifecycle = analyzeLifecycle(entry, sourceCode);
    {
        ...
        if (lifecycle.ini) {
            for (const b of lifecycle.ini.split(",")) {
                if (b === "add_observers")
                    iniParts.push("bind_obs");
                else if (b === "conn_remote")
                    iniParts.push("conn");
                else if (b === "start_timer")
                    iniParts.push("sched");
                else if (b === "configures_defaults")
                    iniParts.push("cfg");
            }
        }
        if (iniParts.length > 0)
            flow.push(`ini:${iniParts.join("")}`);
    }
    // Deinit — tagged index refs di[n] + cleanup descriptors from source
    {
        const diParts = [];
        if (entry.deinits?.length) {
            for (let n = 0; n < Math.min(entry.deinits.length, 3); n++)
                diParts.push(`!di[${n}]`);
        }
        if (lifecycle.di) {
            const cleanups = lifecycle.di.split(",").filter((p) => ["cancel_tasks", "remove_observers", "close_conn", "stop_timer", "release_refs", "cleanup"].includes(p));
            for (const c of cleanups) {
                if (c === "cancel_tasks")
                    diParts.push("clr_tasks");
                else if (c === "remove_observers")
                    diParts.push("clr_obs");
                else if (c === "close_conn")
                    diParts.push("clr_conn");
                else if (c === "stop_timer")
                    diParts.push("clr_timer");
                else if (c === "release_refs")
                    diParts.push("clr_refs");
                else
                    diParts.push(c);
            }
        }
        if (diParts.length > 0)
            flow.push(`di:${diParts.join(",")}`);
    }
    ...
    if (callCount >= 8)
        flow.push("hub");
    else if (callCount === 0 && !entry.stores?.length)
        flow.push("leaf");
    return flow.join("|");
}
/** Build UDLF+ flow for a target/library node — intent + extension capabilities only */
function buildTargetFlow(entry, callerNames) {
    const flow = [];
...
function detectStateMutations(sourceCode) {
    const mutations = [];
    // self.property = ... patterns
    if (/self\.\w+\s*=\s*[^=]/.test(sourceCode))
        mutations.push("self_state");
    // @State/@Published mutations
    if (/@Published\b|@State\b/.test(sourceCode) && /\.\w+\s*=/.test(sourceCode))
        mutations.push("reactive_state");
    // Array/collection mutations
    if (/\.append\(|\.remove|\.insert|\.replaceSubrange/.test(sourceCode))
        mutations.push("collection");
    // Counter/numeric increments
    if (/\+=\s*\d|\-=\s*\d|\.toggle\(\)/.test(sourceCode))
        mutations.push("counter");
    // UserDefaults/persistence
    if (/UserDefaults.*set\(|\.setValue\(/.test(sourceCode))
...
function detectGuardClause(sourceCode) {
    // guard let x = ... else { return }
    const guardLet = sourceCode.match(/guard\s+let\s+(\w+)/);
    if (guardLet)
        return `${guardLet[1]}!=nil`;
    // guard x != nil / guard !x.isEmpty / guard x > 0
    const guardCond = sourceCode.match(/guard\s+(\w+(?:\.\w+)?)\s*(!?=\s*\w+|\.isEmpty|[><=]\s*\d+)/);
    if (guardCond)
        return `${guardCond[1]}${guardCond[2]}`.replace(/\s+/g, "");
    // Generic guard presence
    if (/\bguard\b/.test(sourceCode))
        return "precond";
    return null;
}
/** Detect notification/KVO/delegate propagation chains */
function detectPropagation(sourceCode) {
    if (/NotificationCenter.*\.post\(/.test(sourceCode))
        return "notification";
    if (/delegate\?\.\w+|delegate\./.test(sourceCode))
        return "delegate";
    if (/\$\w+\.send\(|objectWillChange/.test(sourceCode))
        return "publisher";
    if (/\.publisher\b|\.sink\b|\.assign\b/.test(sourceCode))
        return "combine";
    if (/setNeedsLayout|setNeedsDisplay|layoutIfNeeded/.test(sourceCode))
        return "layout";
    return null;
}
/**
 ...
 */
function detectIntelligence(entry, sourceCode) {
    const parts = [];
    if (!sourceCode) {
    ...
    if (/\bcatch\b.*\bretry\b|\bretry\b.*\bcatch\b/i.test(sourceCode))
        parts.push("err:retry");
    else if (/\bthrow\b|\bthrows\b/.test(sourceCode))
        parts.push("err:throw");
    else if (/\btry\?\s/.test(sourceCode))
        parts.push("err:silent");
    else if (/\btry!\s/.test(sourceCode))
        parts.push("err:force⚠");
    ...
    if (/\bfor\b.*\bin\b.*\bfor\b.*\bin\b/s.test(sourceCode))
        parts.push("cost:H"); // nested loops
    else if (/\.sorted\(|\.filter\(.*\.map\(|\.reduce\(/.test(sourceCode))
        parts.push("cost:M");
    else
        parts.push("cost:L");
    // p: — design pattern detection
    if (/shared\s*[:=]|\.shared\b|static\s+let\s+\w+\s*[:=]\s*\w+\(\)/.test(sourceCode))
        parts.push("p:singleton");
    else if (/delegate\??\.|\bDelegate\b.*protocol/.test(sourceCode))
        parts.push("p:delegate");
    else if (/NotificationCenter|\.addObserver|@Published|\.sink\b/.test(sourceCode))
        parts.push("p:observer");
    else if (/func\s+make\w+|static\s+func\s+create/.test(sourceCode))
        parts.push("p:factory");
    else if (/\.build\(\)|Builder\b/.test(sourceCode))
        parts.push("p:builder");
    // @ state-change effects (kept from original)
    const effects = [];
    if (/URLSession|URLRequest|\.fetch/i.test(sourceCode))
        effects.push("net");
    if (/FileManager|write\(/i.test(sourceCode))
        effects.push("disk");
    if (/UserDefaults|CoreData/i.test(sourceCode))
        effects.push("state");
    if (/UIView|SwiftUI|@Published/i.test(sourceCode))
        effects.push("ui");
    if (effects.length > 0)
 ...
 */
function summarizeAccessors(childContexts) {
    const accessors = [];
 ...
 */
function inferExecTag(entry) {
    const name = entry.name.toLowerCase();
    // Variable flavor — infer accessor type from name
    if (entry.flavor === "variable") {
        if (name === "willset" || name.endsWith("[willset]") || name.endsWith(".willset"))
            return "ws";
        if (name === "didset" || name.endsWith("[didset]") || name.endsWith(".didset"))
            return "ds";
        if (name === "getter" || name.endsWith("[get]") || name.endsWith(".get"))
            return "g";
        if (name === "setter" || name.endsWith("[set]") || name.endsWith(".set"))
            return "st";
        // Computed property (variable with executable body but no specific accessor)
        return "g";
    }
    // Initializer flavor
    if (entry.flavor === "initializer")
        return "i";
 ...
 */
function analyzeLifecycle(entry, sourceCode) {
    let ini = null;
    let di = null;
    ...
    if (hasInits || sourceCode) {
        ...
        if (hasInits) {
            ...
            const callsExternal = initDeps.some((d) => !["self", "super"].includes(d.toLowerCase()));
        ...
        if (sourceCode && /init\s*\(/.test(sourceCode)) {
            if (!hasInits)
                initTraits.push("assign_only");
            if (/NotificationCenter.*addObserver|\.observe\(/.test(sourceCode))
                initTraits.push("add_observers");
            if (/URLSession|connect|socket|\.start\(\)/i.test(sourceCode))
                initTraits.push("conn_remote");
            if (/Timer\.|DispatchSource|schedule/i.test(sourceCode))
                initTraits.push("start_timer");
            if (/UserDefaults|\.register\(defaults/i.test(sourceCode))
                initTraits.push("configures_defaults");
            if (/super\.init/.test(sourceCode))
                initTraits.push("calls_super");
        }
        if (initTraits.length > 0)
            ini = [...new Set(initTraits)].slice(0, 4).join(",");
    }
    ...
    if (entry.flavor === "class" || entry.flavor === "actor") {
        ...
        if (sourceCode && /deinit\s*\{/.test(sourceCode)) {
            if (/cancel\(\)|\.cancel\b/i.test(sourceCode))
                deinitTraits.push("cancel_tasks");
            if (/removeObserver|NotificationCenter.*remove/i.test(sourceCode))
                deinitTraits.push("remove_observers");
            if (/close\(\)|disconnect|\.stop\(\)/i.test(sourceCode))
                deinitTraits.push("close_conn");
            if (/invalidate\(\)|Timer.*invalidate/i.test(sourceCode))
                deinitTraits.push("stop_timer");
            if (/nil\b.*=|= nil/i.test(sourceCode))
                deinitTraits.push("release_refs");
            if (deinitTraits.length === 0)
                deinitTraits.push("cleanup");
        }
        if (deinitTraits.length > 0)
            di = [...new Set(deinitTraits)].slice(0, 4).join(",");
    }
    return { ini, di };
}
/** Infer intent from node name, children intents, or library role */
function inferIntent(entry, childContexts, callerNames) {
    ...
    if (!OBJECT_FLAVORS.has(entry.flavor) && entry.flavor !== "target") {
        ...
        const name = entry.name.toLowerCase();
        if (name === "willset" || name.endsWith("[willset]") || name.endsWith(".willset"))
            return "validate_before_set";
        if (name === "didset" || name.endsWith("[didset]") || name.endsWith(".didset"))
            return "react_after_set";
        if (name === "deinit")
            return "cleanup";
        if (entry.flavor === "initializer" || name === "init" || name.startsWith("init("))
            return "initialize";
    ...
    if (entry.flavor === "target") {
        ...
        if (entry.origin === "Apple")
            return inferAppleFrameworkRole(entry.name);
        return "third_party";
    }
    return null;
}
...
function extractTokensFromContexts(contexts, prefix) {
    const results = [];
    const seen = new Set();
...
function inferFunctionIntent(name) {
    const n = name.toLowerCase();
    if (/^(get|fetch|load|read|query|find|search|retrieve)/.test(n))
        return "read";
    if (/^(set|update|save|write|store|put|patch|modify)/.test(n))
        return "write";
    if (/^(create|make|build|init|new|generate|produce)/.test(n))
        return "create";
    if (/^(delete|remove|drop|destroy|clear|reset|purge)/.test(n))
        return "delete";
    if (/^(render|draw|display|show|present|layout)/.test(n))
        return "render";
    if (/^(validate|check|verify|assert|ensure|guard)/.test(n))
        return "validate";
    if (/^(configure|setup|register|bind|connect|attach)/.test(n))
        return "setup";
    if (/^(handle|on[A-Z]|did|will|process|respond)/.test(n))
        return "handle";
    if (/^(parse|decode|encode|serialize|transform|convert|map)/.test(n))
        return "transform";
    if (/^(test|spec|assert|expect)/.test(n))
        return "test";
    return null;
}
function inferStructPurpose(name) {
    const n = name.toLowerCase();
    if (/config|setting|option|preference/.test(n))
        return "configuration";
    if (/request|response|payload|body/.test(n))
        return "data_transfer";
    if (/model|entity|record|data|info/.test(n))
        return "data_model";
    if (/state|view.*state|ui.*state/.test(n))
        return "state_container";
    if (/error|failure/.test(n))
        return "error_type";
    if (/result|outcome/.test(n))
        return "result_wrapper";
    return null;
}
function inferEnumPurpose(name) {
    const n = name.toLowerCase();
    if (/route|screen|destination|page|tab/.test(n))
        return "navigation";
    if (/state|status|phase/.test(n))
        return "state_machine";
    if (/error|failure|issue/.test(n))
        return "error_cases";
    if (/action|event|intent|command/.test(n))
        return "action_dispatch";
    if (/type|kind|category|style|variant/.test(n))
 ...
 */
function inferExtensionCapabilities(mergedSource) {
    ...
    if (/func\s+tableView|func\s+collectionView|func\s+numberOfRows|UITableView|UICollectionView/.test(extSource))
        capabilities.push("table_data_source");
    if (/func\s+textField|UITextFieldDelegate|func\s+textView/.test(extSource))
        capabilities.push("text_input");
    if (/@objc\s+func|#selector|@IBAction/.test(extSource))
        capabilities.push("objc_actions");
    if (/Codable|Decodable|Encodable|func\s+encode\(|init\(from\s+decoder/.test(extSource))
        capabilities.push("coding");
    if (/Equatable|Hashable|Comparable|func\s+==\s*\(/.test(extSource))
        capabilities.push("equality");
    if (/CustomStringConvertible|description\s*:/.test(extSource))
        capabilities.push("debug_description");
    if (/\.snp\.|makeConstraints|NSLayoutConstraint|translatesAutoresizingMask/.test(extSource))
        capabilities.push("layout");
    if (/style|theme|color|font|appearance|backgroundColor/.test(extSource))
        capabilities.push("styling");
    if (/@Published|@State|ObservableObject|Combine/.test(extSource))
        capabilities.push("reactive");
    if (/preview|PreviewProvider|#Preview/.test(extSource))
    ...
    if (capabilities.length === 0) {
        ...
        if (funcCount > 0)
            return `${funcCount}_methods`;
        return null;
    }
    return capabilities.slice(0, 4).join(",");
}
function inferAppleFrameworkRole(name) {
    const roles = {
...
function buildFileSummaries(entries) {
    const byFile = new Map();
    for (const e of entries) {
        if (e.flavor === "target" || !e.location.absPath)
            continue;
    ...
    for (const [filePath, fileEntries] of byFile) {
        ...
        for (const e of fileEntries) {
            const ctx = e.node_context;
            if (!ctx)
                continue;
            // Extract t: or e: value
            for (const seg of ctx.split("|")) {
                if (seg.startsWith("t:") || seg.startsWith("e:"))
                    types.add(seg);
                if (seg.startsWith("i:")) {
                    const val = seg.slice(2);
                    if (val && !intents.includes(val))
                        intents.push(val);
                }
            }
        }
        const parts = [`t:f`]; // f = file
        parts.push(`n:${fileName}`);
        if (intents.length > 0)
            parts.push(`i:${intents.slice(0, 4).join(",")}`);
        if (types.size > 0)
            parts.push(`contains:${[...types].slice(0, 6).join(",")}`);
...
function buildTargetSummaries(entries, childrenOf) {
    const summaries = {};
    for (const e of entries) {
        if (e.flavor !== "target")
            continue;
        ...
        const contained = entries.filter((c) => c.id.startsWith(targetPrefix) && c.flavor !== "target");
        ...
        const existing = e.node_context;
        const parts = existing ? [existing] : [`t:tg|i:${e.origin ? (e.origin === "Apple" ? "apple_sdk" : "third_party") : "internal"}`];
        if (objectCount > 0)
            parts.push(`types:${objectCount}`);
        if (funcCount > 0)
            parts.push(`funcs:${funcCount}`);
        if (intents.length > 0 && !existing)
            parts.push(`scope:${intents.slice(0, 4).join(",")}`);
...
// PERSISTENCE
// ═══════════════════════════════════════════════════════════════════════════════
function persistEnrichedGraph(entries, workspacePath) {
...
// ═══════════════════════════════════════════════════════════════════════════════
async function enrichWithSemanticContext(entries, stealthDir, callbacks, cancellation) {
    clearFileCache();
    ...
    const parents = [];
    for (const e of entries) {
        if (!ELIGIBLE_FLAVORS.has(e.flavor))
            continue;
    ...
    const result = { enrichedCount: 0, cachedCount: 0, failedCount: 0, llmUsed: false };
    const ollamaReady = await isOllamaAvailable();
    result.llmUsed = ollamaReady;
    const progress = { total: totalEligible, completed: 0, cached: 0, llmUsed: ollamaReady };
...
async function processBatch(items, entries, cache, ollamaReady, result, progress, callbacks, cancellation, childrenOf, callersOf) {
    const queue = [...items];
    const inFlight = [];
    while (queue.length > 0 || inFlight.length > 0) {
        ...
        while (inFlight.length < MAX_CONCURRENT && queue.length > 0) {
            ...
            const task = processNode(item.entry, item.index, entries, cache, ollamaReady, result, progress, callbacks, childrenOf, callersOf);
...
async function processNode(entry, index, entries, cache, ollamaReady, result, progress, callbacks, childrenOf, callersOf) {
    ...
    if (isParent) {
        for (const childId of childrenOf.get(entry.id) ?? []) {
            ...
            if (childEntry && childEntry.node_context) {
                childContexts.push(childEntry.node_context);
            }
        }
    }
    ...
    if (isTarget) {
        ...
        if (extensionCount > 0)
            hashInput += `|ext:${extensionCount}`;
        if (childContexts.length > 0)
            hashInput += `|children:${childContexts.length}`;
        if (entry.inits?.length)
            hashInput += `|inits:${entry.inits.join(",")}`;
        if (entry.deinits?.length)
            hashInput += `|deinits:${entry.deinits.join(",")}`;
    }
    const hash = hashContent(hashInput);
    // Cache check
    const cached = cache.entries[entry.id];
    if (cached && cached.hash === hash) {
        entries[index].node_context = cached.context;
        result.cachedCount++;
        progress.cached++;
        progress.completed++;
    ...
    entries[index].node_context = context;
...
//# sourceMappingURL=ollamaBridge.js.map